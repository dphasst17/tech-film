...
const New = () => {
    ...
    return <div className='w-[90%] h-auto grid grid-cols-1 gap-2 mx-auto my-10'>
        ...
        <div className='w-full h-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-6 gap-4'>
            {newFilm !== null && newFilm.slice(0, 6).map((c, i) => <FilmLayout data={c} key={`new-${i}`} />)}
        </div>
    </div>
}
